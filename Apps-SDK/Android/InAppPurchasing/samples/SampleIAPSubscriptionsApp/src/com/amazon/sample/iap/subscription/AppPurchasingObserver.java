...
public class AppPurchasingObserver extends BasePurchasingObserver {
	...
	public void onPurchaseUpdatesResponse(PurchaseUpdatesResponse response) {
		...
		case SUCCESSFUL:
			...
			// No need to save purchase updates offset for subscriptions
			// simplest to just request from Offset.BEGINNING
		...
		case SUCCESSFUL:
			...
			Log.i(TAG,
					"onPurchaseResponse: receipt itemType ("
							...
							+ receipt.getPurchaseToken() + ") subscriptionPeriod ("
	...
	private class PurchaseUpdatesAsyncTask extends
		...
		protected Boolean doInBackground(PurchaseUpdatesData... args) {
			...
			LatestSubscriptionPeriod latestSubscriptionPeriod = new LatestSubscriptionPeriod();
			for (Receipt receipt : receipts) {
				String parentSku = receipt.getSku();
				SubscriptionPeriod subscriptionPeriod = receipt
						.getSubscriptionPeriod();
				Log.i(TAG,
						"PurchaseUpdatesAsyncTask.doInBackground: receipt itemType ("
								+ receipt.getItemType() + ") parentSKU ("
								+ parentSku + ") purchaseToken ("
								+ receipt.getPurchaseToken()
								+ ") subscriptionPeriod (" + subscriptionPeriod
								+ ")");

				// Store latest subscription period
				// (the one with the latest start date) for each Parent SKU
				latestSubscriptionPeriod.putIfLatest(parentSku, receipt);
			}

			Map<String, Receipt> latestSubPeriodPerSKU = latestSubscriptionPeriod
					.getAll();
			for (Entry<String, Receipt> entry : latestSubPeriodPerSKU
					.entrySet()) {
				String parentSku = entry.getKey();
				Receipt receiptWithLatestSubPeriod = entry.getValue();
				SubscriptionPeriod latestSubPeriod = receiptWithLatestSubPeriod
						.getSubscriptionPeriod();
				// If SubscriptionPeriod EndDate is NULL,
				// then subscription has NOT ended yet.
				Date now = new Date();
				boolean isSubscriptionActive = (latestSubPeriod.getStartDate().compareTo(now) < 0) && (latestSubPeriod.getEndDate() == null);

				Log.d(TAG, "PurchaseUpdatesAsyncTask.doInBackground for SKU ("
						+ parentSku + ") latest subscription startDate ("
						+ latestSubPeriod.getStartDate() + ") endDate ("
						+ latestSubPeriod.getEndDate()
						+ "), should we fulfill, is subscription active? = "
						+ isSubscriptionActive);

				String sku = MySKU.valueForParentSKU(parentSku).getSku();
				...
				listener.onPurchaseUpdatesResponseSuccess(userId, sku,
						receiptWithLatestSubPeriod.getPurchaseToken(),
						isSubscriptionActive);
				Log.i(TAG,
			...
			// Subscriptions are NOT revoked as customer would have paid for it
			// at beginning of subscription and is entitled to it for
			// subscription period.
			// Therefore, no need to call getRevokedSkus() for subscriptions
			return true;
		}
	}
	...
	/**
	 * Used to store the latest subscription period data for each SKU.  
	 * This data is not persisted, since it should be updated with every call to
	 * {@link PurchasingManager#initiatePurchaseUpdatesRequest}
	 * which should be called every time the app resumes.
	 */
	protected static class LatestSubscriptionPeriod {
		private Map<String, Receipt> map;

		public LatestSubscriptionPeriod() {
			map = new HashMap<String, Receipt>();
		}

		/**
		 * Stores receipt for parentSku if the receipt has
		 * the latest subscription period.
		 * 
		 * @param parentSku
		 * @param receipt
		 */
		public void putIfLatest(String parentSku, Receipt receipt) {
			if ((parentSku == null) || (receipt == null))
				return;

			Receipt latestSaved = get(parentSku);
			if (latestSaved == null) {
				map.put(parentSku, receipt);
			} else {
				SubscriptionPeriod latestSavedPeriod = latestSaved
						.getSubscriptionPeriod();
				SubscriptionPeriod subPeriod = receipt.getSubscriptionPeriod();

				Date subPeriodStartDate = subPeriod.getStartDate();
				Date latestSavedStartDate = latestSavedPeriod.getStartDate();
				// If subscription's startDate >= latest saved subscription's startDate
				// save it because this is the latest subscription
				if (subPeriodStartDate.equals(latestSavedStartDate)
						|| subPeriodStartDate.after(latestSavedStartDate)) {
					map.put(parentSku, receipt);
				}
			}
		}
		...
		/**
		 * Get receipt for parentSku
		 */
		public Receipt get(String parentSku) {
			return map.get(parentSku);
		}

		/**
		 * Get all parentSku -> Receipt mappings
		 */
		public Map<String, Receipt> getAll() {
