...
import com.amazon.sample.iap.entitlement.AppPurchasingObserver.PurchaseData;
import com.amazon.sample.iap.entitlement.AppPurchasingObserver.PurchaseDataStorage;
...
public class MainActivity extends Activity implements
	...
	public void onBuyAccessToLevel2Click(View view) {
		String requestId = PurchasingManager
				.initiatePurchaseRequest(MySKU.LEVEL2.getSku());
	...
	public void onItemDataResponseSuccessful(Map<String, Item> itemData) {
		for (Entry<String, Item> entry : itemData.entrySet()) {
			...
			Log.i(TAG, "onItemDataResponseSuccessful: sku (" + sku + ") item ("
					+ item + ")");
			if (MySKU.LEVEL2.getSku().equals(sku)) {
	...
	private void disableButtonsForUnavailableSkus(Set<String> unavailableSkus) {
		for (String unavailableSku : unavailableSkus) {
			...
			if (MySKU.LEVEL2.getSku().equals(unavailableSku)) {
				Log.i(TAG,
						"disableButtonsForUnavailableSkus: unavailableSKU ("+unavailableSku+"), disabling buyLevel2Button");
