...
6. Have your Activity class implement the AppPurchasingObserverListener interface.  
   ...
   This means that the IAP SDK has been initialized successfully and that you are currently in sandbox (testing) mode.
    ...
    I/MyApp(27169): onSdkAvailable: isSandboxMode: true
...
10. Copy the onBuyOrangeClick(View view) method from this sample app's MainActivity class into your app's activity class.  
    Now, add a button to call this method.  If it's easier, you can just copy the buy_orange_button button from this sample app's res/layout/activity_main.xml file into your app.  
	...
	public void onBuyOrangeClick(View view) {
		String requestId = PurchasingManager.initiatePurchaseRequest(MySKU.ORANGE.getSku());
		
		PurchaseData purchaseData = purchaseDataStorage.newPurchaseData(requestId);
		
		Log.i(TAG, "onBuyOrangeClick: requestId (" + requestId + ") requestState (" + purchaseData.getRequestState() + ")");
	}
